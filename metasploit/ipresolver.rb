...
class Metasploit3 < Msf::Post
	...
	def initialize(info={})
		super(update_info(info,
			'Name'                 => "IP Resolver",
			'Description'          => %q{ This module uses Railgun, calling the gethostbyaddr
		...
		register_options(
		[
			OptAddressRange.new('RHOSTS', [true, 'IP address or range to query', '127.0.0.1'])
		], self.class)
	end
	...
	def run
		...
		if modhandle['return'] == 0
			...
			procaddr = client.railgun.kernel32.GetProcAddress(modhandle['return'],'gethostbyaddr')
